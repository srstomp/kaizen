...
func TestRunEvaluationUsesIsolation(t *testing.T) {
	failureCase := FailureCase{
		...
		Evidence: FailureEvidence{
			TaskSpec:     "Implement a feature",
			WhatWasBuilt: "Feature implemented",
		},
		EvalCriteria: []EvalCriterion{
			{Type: "model-based", Check: "spec_compliance"},
		},
	}
	...
	// With model-based spec_compliance grader using stub evaluation, should pass
	for i, run := range result.Runs {
		if !run {
			t.Errorf("Run %d failed unexpectedly (spec_compliance should pass with stub)", i)
		}
	}
}
