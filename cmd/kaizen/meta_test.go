...
func TestRunMetaCommandErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := runMetaCommand(tt.suite, tt.agent, 0, metaDir, true)
			if err == nil {
...
// TestFormatAgentPrompt tests the prompt formatting for different agents
func TestFormatAgentPrompt(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Brainstormer agent with task description",
			agentName: "yokay-brainstormer",
			input: TaskInput{
				TaskTitle:          "Add user authentication",
				TaskDescription:    "Need to add login functionality",
				AcceptanceCriteria: []string{"Users can log in", "Sessions persist"},
			},
			expectedParts: []string{
				"Task Title: Add user authentication",
				"Task Description: Need to add login functionality",
				"Acceptance Criteria:",
				"- Users can log in",
				"- Sessions persist",
			},
		...
		{
			name:      "Quality reviewer agent with implementation",
			agentName: "yokay-quality-reviewer",
			input: TaskInput{
				TaskTitle:      "Fix login bug",
				Implementation: "function login() { return true; }",
			},
			expectedParts: []string{
		...
		{
			name:      "Spec reviewer agent with all fields",
			agentName: "yokay-spec-reviewer",
			input: TaskInput{
				TaskTitle:          "Refactor auth module",
				TaskDescription:    "Improve the authentication system",
				AcceptanceCriteria: []string{"Code is cleaner", "Tests pass"},
				Implementation:     "// New auth code",
			},
			expectedParts: []string{
				"Task Title: Refactor auth module",
				"Task Description: Improve the authentication system",
				"Acceptance Criteria:",
				"- Code is cleaner",
				"- Tests pass",
				"Implementation:",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := formatAgentPrompt(tt.agentName, tt.input)
...
// TestExtractVerdict tests verdict extraction from agent output
...
- No tests included`,
		...
		{
			name: "REFINED verdict in output",
...
The requirements have been clarified.`,
			expectedVerdict: "REFINED",
		},
		{
			name: "NEEDS_INPUT verdict",
			output: `Cannot proceed without more information.

Verdict: NEEDS_INPUT`,
			expectedVerdict: "NEEDS_INPUT",
		},
		{
			name: "SKIP verdict",
			output: `This test case is not applicable.
...
All checks completed successfully.`,
		...
		{
			name: "Verdict with surrounding text",
			output: `After thorough analysis, the verdict: FAIL - the code needs improvement.`,
			expectedVerdict: "FAIL",
		},
		{
			name: "Multiple verdicts - first wins",
			output: `Initial verdict: PASS

Actually, on second thought: FAIL`,
			expectedVerdict: "PASS",
		},
		{
			name: "No verdict keyword",
			output: `This is some output without any verdict keyword in it.
...
Problems detected.`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := extractVerdict(tt.output)
			if result != tt.expectedVerdict {
				t.Errorf("Expected verdict %q, got %q", tt.expectedVerdict, result)
			}
		})
	}
}

// TestFormatAgentPromptBasic tests that formatAgentPrompt properly formats input
func TestFormatAgentPromptBasic(t *testing.T) {
	// Test that formatAgentPrompt properly formats input
	input := TaskInput{
		TaskTitle:       "Test task",
		TaskDescription: "Description",
	}

	prompt := formatAgentPrompt("test-agent", input)
	if prompt == "" {
		t.Error("formatAgentPrompt returned empty string")
	}
	if !strings.Contains(prompt, "Test task") {
		t.Error("formatAgentPrompt should include task title")
	}
}

// TestConfirmMetaExecution tests API call estimation and cost calculation
func TestConfirmMetaExecution(t *testing.T) {
	// Setup: Create temp directory with test eval files
	tmpDir := t.TempDir()
	...
	// Write an eval.yaml with known test cases
	sampleEval := `agent: yokay-test-agent
consistency_threshold: 0.95
...
`
	...
	t.Run("Estimates API calls correctly", func(t *testing.T) {
		// Expected: 3 + 5 = 8 total API calls
		// This test verifies the estimation logic without requiring stdin interaction
		evalFiles := []string{evalPath}

		// Test with confirm=true (should skip prompt and succeed)
		err := confirmMetaExecution(evalFiles, 0, true)
		if err != nil {
			t.Errorf("confirmMetaExecution with confirm=true should not error, got: %v", err)
		}
	})

	t.Run("Confirm flag bypass", func(t *testing.T) {
		// When confirm=true, should skip prompt
		evalFiles := []string{evalPath}

		err := confirmMetaExecution(evalFiles, 0, true)
		if err != nil {
			t.Errorf("Expected no error with confirm=true, got: %v", err)
		}
	})

	t.Run("Uses k override when provided", func(t *testing.T) {
		// When k override is provided, it should use that instead of YAML k values
		evalFiles := []string{evalPath}

		// With k=10, expected calls = 2 test cases * 10 = 20
		err := confirmMetaExecution(evalFiles, 10, true)
		if err != nil {
			t.Errorf("Expected no error with k override, got: %v", err)
		}
	})

	t.Run("Uses default k when not specified", func(t *testing.T) {
		// Create eval without k values
		agentDir2 := filepath.Join(tmpDir, "agents", "test-agent-alt")
		err := os.MkdirAll(agentDir2, 0755)
		if err != nil {
			t.Fatalf("Failed to create agent dir: %v", err)
		}

		sampleEval2 := `agent: yokay-test-agent-alt
consistency_threshold: 0.95
...
test_cases:
  - id: TST-003
    ...
    rationale: "Should pass"
`
		evalPath2 := filepath.Join(agentDir2, "eval.yaml")
		err = os.WriteFile(evalPath2, []byte(sampleEval2), 0644)
		if err != nil {
			t.Fatalf("Failed to write test eval.yaml: %v", err)
		}

		evalFiles := []string{evalPath2}

		// Should use default k=5
		err = confirmMetaExecution(evalFiles, 0, true)
		if err != nil {
			t.Errorf("Expected no error with default k, got: %v", err)
		}
	})

	t.Run("Error on invalid eval file", func(t *testing.T) {
		evalFiles := []string{"/nonexistent/eval.yaml"}

		err := confirmMetaExecution(evalFiles, 0, true)
		if err == nil {
			t.Error("Expected error for nonexistent file, got nil")
		}
		if !strings.Contains(err.Error(), "loading eval file") {
			t.Errorf("Expected error about loading file, got: %v", err)
		}
	})
}
...
`
	...
	// Execute - should not return error (with confirm=true to skip prompt)
	err = runMetaCommand("", "test-agent", 0, metaDir, true)
	if err != nil {
